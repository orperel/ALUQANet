...
class AluQACount(Model):
    ...
    def __init__(self,
        ...
        self._span_weights_predictor = torch.nn.Linear(bert_dim, 1)
        ...
        if "counting" in self.answering_abilities:
            ...
            self._count_passage_span_start_predictor = torch.nn.Linear(bert_dim, 2)
            self._count_passage_span_end_predictor = torch.nn.Linear(bert_dim, 2)
    ...
    def summary_vector(self, encoding, mask, in_type="passage"):
        if in_type == "passage":
            ...
            # Shape: (seqlen)
            alpha = self._span_weights_predictor(encoding).squeeze()
        else:
    ...
    def forward(self,  # type: ignore
        ...
        if "counting" in self.answering_abilities:
            count_number, max_prob, min_prob, best_passage_spans, span_select_prob_combined = self._count_module(
                passage_out, passage_mask, question_vector)
            answer_as_counts = answer_as_counts.squeeze(1)
            gold_count_mask = (answer_as_counts != -1).long()
            count_number = util.replace_masked_values(count_number, gold_count_mask, 0)

            self.print_selection(answer_as_counts, metadata, best_passage_spans, span_select_prob_combined)
        ...
        #     marginal_log_likelihood = log_marginal_likelihood_for_count
        #
                    ...
                    if predicted_ability_str == "passage_span_extraction":
                        ...
                        if self.arithmetic == "base":
                            answer_json["value"], answer_json["numbers"] = \
                                self._base_arithmetic_prediction(original_numbers, number_indices[i],
                                                                 best_signs_for_numbers[i])
                        else:
    ...
    def print_selection(self, answer_as_counts, metadata, best_passage_spans, span_select_prob_combined):
        for i in range(answer_as_counts.size()[0]):
            print()
            print()
            print('Answer: ', metadata[i]['answer_texts'][0])
            selected_spans = best_passage_spans[i]
            selected_spans_start = selected_spans[:, 0]
            selected_spans_end = selected_spans[:, 1]
            probabilities = span_select_prob_combined[i]
            span_end_probability_zip = list(zip(selected_spans_end, probabilities))
            for token_index, token in enumerate(metadata[i]['question_passage_tokens']):
                if token_index in selected_spans_start:
                    print(colored('[', 'red'), " ", end='')

                end_index_prob_zip = list(filter(lambda zipped: zipped[0] == token_index, span_end_probability_zip))
                if len(end_index_prob_zip) > 0:
                    print(colored(token, 'red'), " ", end='')
                    print(colored('(' + "%.4f" % end_index_prob_zip[0][1] + '%)', 'red'), " ", end='')
    ...
    def _count_module(self, passage_out, passage_mask, question_vector):
        # Shape: (batch_size, k, 2), (batch_size, k)
        best_passage_spans, best_passage_spans_probs = self._get_passage_spans(passage_out, passage_mask)
        # Shape: (batch_size, k, bert_dim)
        passage_spans_summary_vectors = self.passage_spans_to_summary_vectors(best_passage_spans, passage_out)
        # Shape: (batch_size, k, 2 * bert_dim)
        count_predictor_input = torch.cat((passage_spans_summary_vectors, question_vector.unsqueeze(1).repeat(1,
                                                                                                              passage_spans_summary_vectors.size()[
                                                                                                                  1],
                                                                                                              1)), -1)
        # Shape: (batch_size, k, 2)
        select_logits = self._count_number_predictor(count_predictor_input)
        # Shape: (batch_size, k)
        select_probs = torch.nn.functional.softmax(select_logits, -1)[:, :, 1]

        # Shape: (batch_size, k)
        span_select_prob_combined = best_passage_spans_probs * select_probs
        # Info about the best count number prediction
        # Shape: (batch_size,)
        count_number = torch.sum(span_select_prob_combined, 1)
        max_prob = torch.max(span_select_prob_combined)
        min_prob = torch.min(span_select_prob_combined)
        return count_number, max_prob, min_prob, best_passage_spans, span_select_prob_combined

    def _get_passage_spans(self, passage_out, passage_mask):
        # Shape: (batch_size, passage_length, 2)
        passage_span_start_logits = self._count_passage_span_start_predictor(passage_out)
        passage_span_start_probs = torch.nn.functional.softmax(passage_span_start_logits, -1)
        # Shape: (batch_size, passage_length)
        passage_span_start_probs_masked = util.replace_masked_values(passage_span_start_probs[:, :, 1], passage_mask, 0)

        # Shape: (batch_size, passage_length)
        passage_span_end_logits = self._count_passage_span_end_predictor(passage_out)
        passage_span_end_probs = torch.nn.functional.softmax(passage_span_end_logits, -1)
        # Shape: (batch_size, passage_length)
        passage_span_end_probs_masked = util.replace_masked_values(passage_span_end_probs[:, :, 1], passage_mask, 0)

        # Shape: (batch_size, 2)
        best_passage_spans, best_passage_spans_probs = self.get_topk_spans(passage_span_start_probs_masked,
                                                                           passage_span_end_probs_masked, 10)
        return best_passage_spans, best_passage_spans_probs

    def get_topk_spans(self, span_start_probs: torch.Tensor, span_end_probs: torch.Tensor, k: int) -> torch.Tensor:
        ...
        max_span_length = 10
        topk_span_start = torch.topk(span_start_probs, k)

        topk_spans = torch.zeros((batch_size, k, 2), device=device).int()
        topk_probs = torch.zeros((batch_size, k), device=device)
        for batch in range(batch_size):
            for i in range(k):
                start_index = topk_span_start.indices[batch][i]
                start_prob = topk_span_start.values[batch][i]

                best_end_token = torch.topk(span_end_probs[batch][start_index: start_index + max_span_length], 1)
                end_index = start_index + best_end_token.indices[0]
                end_prob = best_end_token.values[0]

                topk_spans[batch][i] = torch.tensor([start_index, end_index], device=device)
                topk_probs[batch][i] = start_prob * end_prob

        return topk_spans, topk_probs

    def passage_spans_to_summary_vectors(self, best_passage_spans, passage_out):
        batch_size, k, _ = best_passage_spans.size()
        bert_dim = passage_out.size()[-1]
        summary_vectors = torch.zeros((batch_size, k, bert_dim), device=passage_out.device)
        for batch in range(batch_size):
            for i in range(k):
                start, end = best_passage_spans[batch][i]
                mask = torch.zeros((passage_out.size()[1],), device=passage_out.device)
                mask[start:end + 1] = 1
                span_summary_vector = self.summary_vector(passage_out[batch], mask, "span")
                summary_vectors[batch][i] = span_summary_vector
    ...
    def _count_prediction(self, best_count_number):
        predicted_count = round(float(best_count_number.detach().cpu().numpy()))
