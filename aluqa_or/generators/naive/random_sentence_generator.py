...
class RandomSentenceGenerator:
    ...
    def __init__(self, ner_tokens_generator, content_sentences_path):
        self.ner_tokens_generator = ner_tokens_generator
        self.corpus = self._load_corpus(content_sentences_path)
        self.possible_placeholders = self.ner_tokens_generator.available_ner_categories() + ['GOAL']
        self.pcfg = PCFGGenerator()
    ...
    def sample(self, metadata, answer):
        ...
        while total_countable_entities < answer:
            golden_sentence, spans, span_indices, template_sentence = self._synthesize_golden_sentence(metadata)
