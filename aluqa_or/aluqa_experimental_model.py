...
@Model.register("aluqa_experimental")
class ALUQAExperimental(Model):
    """
    ...
    def summary_vector(self, encoding, mask, in_type = "passage"):
        ...
        elif in_type == "sentence":
            # Shape: (batch_size, seqlen)
            batch_size = encoding.shape[0]
            sentence_count = encoding.shape[1]
            seq_len = encoding.shape[2]
            alpha = self._sentence_weights_predictor(encoding).reshape(batch_size, sentence_count, seq_len)
        else:
    ...
    def extract_sentence_embeddings(self, question_sentences_tokens):
        ...
        sentence_count = sentences_tokens.shape[1]
        batch_size = sentences_tokens.shape[0]

        # Shape: (batch_size x sentence_count, bert_dim) ; self.BERT is a BertModel
        ...
        sep_indices = torch.sum(pad_mask_flat, dim=1) - 1
        sep_indices = torch.max(sep_indices, torch.zeros_like(sep_indices)) # Surpass null sentences due to batch padding
    ...
    def forward(self,  # type: ignore
                question_passage: Dict[str, torch.LongTensor],
                sentences_mask: torch.LongTensor,               # New
                sentences_tokens: torch.LongTensor,             # New
                question_sentences_tokens: torch.LongTensor,    # New
                number_indices: torch.LongTensor,
                mask_indices: torch.LongTensor,
                num_spans: torch.LongTensor = None,
                answer_as_passage_spans: torch.LongTensor = None,
        ...
        bert_out, _ = self.BERT(question_passage_tokens, seqlen_ids, pad_mask, output_all_encoded_layers=False)
        # Shape: (batch_size, qlen, bert_dim)
        question_end = max(mask[:,1])   # Contains the index for last question token: Shape (batch_size, )
        question_out = bert_out[:,:question_end]    # Contains question token embeddings + some unfiltered embeddings from passage
        # Shape: (batch_size, qlen)
        question_mask = question_mask[:,:question_end]  # Crop mask to match question_out
        # Shape: (batch_size, out)
        question_vector = self.summary_vector(question_out, question_mask, "question")
        ...
        with torch.no_grad():
            sentences_out, sentences_embedding_mask = self.extract_sentence_embeddings(question_sentences_tokens)
        sentences_vectors = self.summary_vector(sentences_out, sentences_embedding_mask, "sentence")

        output_dict = {}

        if self.experimental_mode == 'original':
            answer_ability_log_probs, arithmetic_logits,\
            best_answer_ability,\
            best_count_number,\
            best_expression,\
            best_passage_span, best_question_span, best_signs_for_numbers,\
            count_number_log_probs, number_indices, number_mask,\
            number_sign_log_probs,\
            passage_span_end_log_probs, passage_span_start_log_probs,\
            question_span_end_log_probs, question_span_start_log_probs = self.calculate_original_loss_components(
                batch_size, number_indices, pad_mask, passage_mask, passage_out, passage_vector, question_mask,
                question_out, question_vector)

            del passage_out, question_out
            # If answer is given, compute the loss.
            self.calculate_original_loss(answer_ability_log_probs, answer_as_counts, answer_as_expressions,
                                         answer_as_expressions_extra, answer_as_passage_spans, answer_as_question_spans,
                                         arithmetic_logits, count_number_log_probs, number_mask, number_sign_log_probs,
                                         output_dict, passage_span_end_log_probs, passage_span_start_log_probs,
                                         question_span_end_log_probs, question_span_start_log_probs)

            if output_dict["loss"] > 10:
                print(output_dict)

        elif self.experimental_mode == 'expected_count_per_sentence':
            count_number, count_per_sentence_logits, valid_sentences_mask = \
                self._count_module_per_sentence(passage_vector, sentences_vectors)
            answer_as_counts = answer_as_counts.squeeze(1)
            gold_count_mask = (answer_as_counts != -1).long()
            count_number = util.replace_masked_values(count_number, gold_count_mask, 0)

            del passage_out, question_out, sentences_out

            output_dict["loss"] = loss_utils.count_loss(answer_as_counts, count_number,
                                                        count_per_sentence_logits, valid_sentences_mask,
                                                        self.entropy_loss_weight)
            best_count_number = count_number
    ...
    def calculate_original_loss(self, answer_ability_log_probs, answer_as_counts, answer_as_expressions,
                                answer_as_expressions_extra, answer_as_passage_spans, answer_as_question_spans,
                                arithmetic_logits, count_number_log_probs, number_mask, number_sign_log_probs,
                                output_dict, passage_span_end_log_probs, passage_span_start_log_probs,
                                question_span_end_log_probs, question_span_start_log_probs):
    ...
    def calculate_original_loss_components(self, batch_size, number_indices, pad_mask, passage_mask, passage_out,
                                           passage_vector, question_mask, question_out, question_vector):

        answer_ability_log_probs = None
        arithmetic_logits = None
        best_answer_ability = None
        best_count_number = None
        best_expression = None
        best_passage_span = None
        best_question_span = None
        best_signs_for_numbers = None
        count_number_log_probs = None
        number_indices = None
        number_mask = None
        number_sign_log_probs = None
        passage_span_end_log_probs = None
        passage_span_start_log_probs = None
        question_span_end_log_probs = None
        question_span_start_log_probs = None
        ...
        return answer_ability_log_probs, arithmetic_logits, best_answer_ability, best_count_number, best_expression,\
               best_passage_span, best_question_span, best_signs_for_numbers, count_number_log_probs, number_indices,\
               number_mask, number_sign_log_probs, passage_span_end_log_probs, passage_span_start_log_probs,\
               question_span_end_log_probs, question_span_start_log_probs
    ...
    def _count_log_likelihood(self, answer_as_counts, count_number_log_probs):
        ...
        log_likelihood_for_counts = \
            util.replace_masked_values(log_likelihood_for_counts, gold_count_mask, -1e-7)
        # Shape: (batch_size, )
