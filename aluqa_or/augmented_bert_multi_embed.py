...
@Model.register("nabert_multi")
class NumericallyAugmentedBERTMulti(Model):
    """
    ...
    def __init__(self,
        ...
        if self.bert_layers is None:
            self.bert_layers = (1, 4, 5, 6, 10, 11)
        ...
        bert_dim = self.BERT.pooler.dense.out_features * len(self.bert_layers)
    ...
    def extract_summary_vecs(self,  # type: ignore
        ...
        bert_out, _ = self.BERT(question_passage_tokens, seqlen_ids, pad_mask, output_all_encoded_layers=True)
        all_bert_layers_out = torch.stack(bert_out)
        all_bert_layers_out = all_bert_layers_out.squeeze(dim=1)
        # Shape: (batch_size, qlen, bert_dim)
        question_end = max(mask[:, 1])
        question_out = all_bert_layers_out[:, :question_end]
        # Shape: (batch_size, qlen)
        question_mask = question_mask[:, :question_end]
        question_mask = question_mask.repeat(all_bert_layers_out.shape[0], 1)
        # Shape: (batch_size, out)
        question_vector = self.summary_vector(question_out, question_mask, "question")

        passage_out = all_bert_layers_out
        del bert_out

        passage_mask = passage_mask.repeat(all_bert_layers_out.shape[0], 1)
    ...
    def forward(self,  # type: ignore
        ...
        all_bert_layers = torch.stack(bert_out).permute(1, 2, 0, 3)
        bert_dim = all_bert_layers.shape[2]
        if len(self.bert_layers) != bert_dim:
            all_bert_layers = all_bert_layers[:,:,self.bert_layers]
        all_bert_layers = all_bert_layers.reshape(batch_size, max_seqlen, -1)

        # Shape: (batch_size, qlen, bert_dim)
        question_end = max(mask[:,1])   # Contains the index for last question token: Shape (batch_size, )
        question_out = all_bert_layers[:,:question_end]    # Contains question token embeddings + some unfiltered embeddings from passage
        # Shape: (batch_size, qlen)
        question_mask = question_mask[:,:question_end]  # Crop mask to match question_out

        # question_mask_all_layers = question_mask.unsqueeze(dim=1).repeat(1, bert_dim, 1).reshape(batch_size, -1)
        # question_out_all_layers = question_out.reshape(batch_size, -1, question_out.shape[-1])

        # Shape: (batch_size, out)
        question_vector = self.summary_vector(question_out, question_mask, "question")

        passage_out = all_bert_layers
        # passage_mask_all_layers = passage_mask.unsqueeze(dim=1).repeat(1, bert_dim, 1).reshape(batch_size, -1)
        # passage_out_all_layers = passage_out.reshape(batch_size, -1, passage_out.shape[-1])
        del bert_out
        del all_bert_layers
