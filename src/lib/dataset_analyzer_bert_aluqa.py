...
def get_model_prediction_info(prediction, metric):
    ...
    return em_correct_label, f1_correct_label, f1_score_label, answer_type_correct

nabertplus_model, config = load_model(model_path='/home/itaysofer/Desktop/Drop/run/nabertplus_count/serialization/model.tar.gz',
...
InstanceLabels = namedtuple('InstanceLabels',
                            ['nabertplus_em_correct', 'nabertplus_f1_score', 'nabertplus_f1_correct', 'nabertplus_answer_type_correct',
                             'aluqa_em_correct', 'aluqa_f1_score', 'aluqa_f1_correct', 'aluqa_answer_type_correct',
                             'answer_types', 'question_tokens', 'answer_texts', 'passage_numbers', 'passage_tokens', 'answer_content'])
...
for instance_idx, instance in enumerate(instances):
    ...
    nabertplus_prediction = nabertplus_model(**model_input)
    nabertplus_metric = nabertplus_model.get_metrics(reset=True)
    nabertplus_em_correct_label, nabertplus_f1_correct_label, nabertplus_f1_score, nabertplus_answer_type_correct =\
        get_model_prediction_info(nabertplus_prediction, nabertplus_metric)

    aluqa_prediction = aluqa_model(**model_input)
    aluqa_metric = aluqa_model.get_metrics(reset=True)
    aluqa_em_correct_label, aluqa_f1_correct_label, aluqa_f1_score, aluqa_answer_type_correct = \
        get_model_prediction_info(aluqa_prediction, aluqa_metric)
    ...
    question_vector, passage_vector = nabertplus_model.extract_summary_vecs(**model_input)
    question_vector = question_vector.squeeze()
    passage_vector = passage_vector.squeeze()

    instance_labels = InstanceLabels(nabertplus_em_correct=nabertplus_em_correct_label,
                                     nabertplus_f1_score=nabertplus_f1_score,
                                     nabertplus_f1_correct=nabertplus_f1_correct_label,
                                     nabertplus_answer_type_correct=nabertplus_answer_type_correct,
                                     aluqa_em_correct=aluqa_em_correct_label,
                                     aluqa_f1_score=aluqa_f1_score,
                                     aluqa_f1_correct=aluqa_f1_correct_label,
                                     aluqa_answer_type_correct=aluqa_answer_type_correct,
...
writer = SummaryWriter("tb_data_analysis/nabert_aluqa_" + datetime.now().strftime("%Y%m%d-%H%M%S"))
