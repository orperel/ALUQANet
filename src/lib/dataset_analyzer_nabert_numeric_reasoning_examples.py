...
from aluqa_or.spans.augmented_bert_plus_synthetic import SyntheticNABERTPlus
from allennlp.modules.span_extractors.self_attentive_span_extractor import SelfAttentiveSpanExtractor
from aluqa_or.spans.data_processing_synthetic import PickleReader, BertDropTokenizer

from allennlp.data.vocabulary import Vocabulary

from src.lib.inference_utils import load_model
...
def load_aluqa_model(weights_path, model_path):
    device_num = 0
    device = torch.device('cuda:%d' % device_num)

    model, config = load_model(weights_path=weights_path, model_path=model_path)

    # if type == 'SyntheticNABERTPlus':
    #     model = SyntheticNABERTPlus(Vocabulary(),
    #                                 'bert-base-uncased',
    #                                 special_numbers=[100, 1],
    #                                 answering_abilities=["counting"],
    #                                 span_extractor=SelfAttentiveSpanExtractor(input_dim=768))
    # else:
    #     model = NumericallyAugmentedBERTPlus(Vocabulary(),
    #                                          'bert-base-uncased',
    #                                          special_numbers=[100, 1],
    #                                          answering_abilities=["counting"])
...
experiment_name = 'synthetic_nabertplus_endpoint_generated_lr7_step2'
model = load_aluqa_model(weights_path=f'../results/best_models/{experiment_name}/best.th',
                         model_path=f'../results/best_models/{experiment_name}/model.tar.gz')
# model = load_aluqa_model(weights_path='../results/best_models/synthetic_nabertplus_selfattentive_generated_step2/best.th', type='SyntheticNABERTPlus')
# model = load_aluqa_model(weights_path='../results/best_models/synthetic_nabertplus_selfattentive_joined_w0_1/best.th', type='SyntheticNABERTPlus')

# instances = create_nabert_reader(data_path='/home/itaysofer/Desktop/Drop/data/drop_dataset_train.json')
instances = create_aluqa_reader(data_path='../../data/itay/drop_dataset/drop_dataset_dev.pickle',
                                question_type=["count"],
                                max_span_length=10,
...
with torch.no_grad():
    for instance_idx, instance in enumerate(instances):
        ...
        question_attention, passage_attention = model.extract_summary_attention_vecs(**model_input)
        question_attention = question_attention.squeeze()
        passage_attention = passage_attention.squeeze()
        ...
        all_tokens = instance.fields['question_passage'].tokens
        question_range = instance.fields['mask_indices'].field_list[0].sequence_index,\
                         instance.fields['mask_indices'].field_list[1].sequence_index
        passage_range = instance.fields['mask_indices'].field_list[1].sequence_index,\
                        instance.fields['mask_indices'].field_list[2].sequence_index

        question_attendable_tokens = all_tokens[question_range[0]:question_range[1]]
        passage_attendable_tokens = all_tokens[passage_range[0]:passage_range[1]]

        q_attention = {str(a): b.item() for a,b in zip(question_attendable_tokens, question_attention)}
        p_attention = {str(a): b.item() for a, b in zip(passage_attendable_tokens, passage_attention)}

        q_attention_sorted = sorted(q_attention.items(), key=lambda e: e[1], reverse=True)
        p_attention_sorted = sorted(p_attention.items(), key=lambda e: e[1], reverse=True)

        results.append(dict(
            question_id=entry['question_id'],
            passage_id=entry['passage_id'],
            passage=instance.fields['metadata'].metadata['original_passage'],
            question=instance.fields['metadata'].metadata['original_question'],
            question_attention=q_attention,
            passage_attention=p_attention,
            question_attention_sorted=q_attention_sorted,
            passage_attention_sorted=p_attention_sorted
        ))

print('Writing results to disk..')
with open(f'{experiment_name}_attention_log.json', 'w') as outfile:
