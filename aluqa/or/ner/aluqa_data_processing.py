...
# @Tokenizer.register("aluqa-drop")
# class DropNERTokenizer(Tokenizer):
#     def __init__(self, tagger_model: str):
#
#         super(DropNERTokenizer, self).__init__()
#
...
#         else:
#             raise ValueError('Unsupported NER Tagger module. '
#                              'Please use \"elmo_peters_2018\" or \"fine_grained_elmo_peters_2018\"')
#
#     @overrides
#     def tokenize(self, text: str) -> List[Token]:
#         words, tags = self.ner_tagger.predict(sentence=text)
#         return [Token(token) for token in self.tokenizer.tokenize(text)]


@TokenIndexer.register("aluqa-drop")
class DropNERTokenIndexer(NerTagIndexer):
...
@DatasetReader.register("aluqa-drop")
class DropNERReader(DatasetReader):
    def __init__(self,
        ...
        super(DropNERReader, self).__init__(lazy)
            ...
            for question_answer in passage_info["qa_pairs"]:
                ...
                if "answer" in question_answer:
                    if self.answer_type is not None and get_answer_type(
                            question_answer['answer']) not in self.answer_type:
                        continue
    ...
    def text_to_instance(self,
        ...
        if answer_annotations:
            for annotation in answer_annotations:
                tokenized_spans = [[token.text for token in self.tokenizer.tokenize(answer)] for answer in
                                   annotation['spans']]
            ...
            # Add answer fields
            passage_span_fields: List[Field] = [SpanField(span[0], span[1], question_passage_field) for span in
                                                valid_passage_spans]
            if not passage_span_fields:
                passage_span_fields.append(SpanField(-1, -1, question_passage_field))
            fields["answer_as_passage_spans"] = ListField(passage_span_fields)

            question_span_fields: List[Field] = [SpanField(span[0], span[1], question_passage_field) for span in
