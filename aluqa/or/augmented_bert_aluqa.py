...
@Model.register("nabert_aluqa")
class NumericallyAugmentedBERTALUQA(Model):
    """
...
#             arithmetic_summary = self.summary_vector(question_out, question_mask, "arithmetic")
    ...
    def _count_module_per_sentence(self, passage_vector, sentences_vectors):
        ...
        valid_sentences_mask = (1 - (torch.sum(sentences_vectors, dim=2) == 0)).unsqueeze(-1)
    ...
    def _count_regression(self, answer_as_counts, count_number_log_probs):
        # Count answers are padded with label -1,
        # so we clamp those paddings to 0 and then mask after `torch.gather()`.
        # Shape: (batch_size, # of count answers)
        gold_count_mask = (answer_as_counts != -1).long()
        ...
        gold_count_mask = gold_count_mask.squeeze().float()
        clamped_gold_counts = clamped_gold_counts.squeeze().float()
        batch_dim = count_number_log_probs.shape[0]
        sentences_count = count_number_log_probs.shape[1]
        count_number_log_probs = count_number_log_probs.reshape(batch_dim, sentences_count) # ~ squeeze()
        pred_count = torch.sum(count_number_log_probs, dim=1) * gold_count_mask
        regression_loss = F.mse_loss(input=pred_count, target=clamped_gold_counts, reduction='none')
        best_count = torch.round(pred_count)
        return regression_loss, best_count
